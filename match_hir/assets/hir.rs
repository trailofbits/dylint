    ...
    pub fn suggestion_position(&self) -> (LifetimeSuggestionPosition, Span) {
        if self.ident.name == kw::Empty {
            if self.ident.span.is_empty() {
                (
                    LifetimeSuggestionPosition::ElidedPathArgument,
                    self.ident.span,
                )
            } else {
                (
                    LifetimeSuggestionPosition::ElidedPath,
                    self.ident.span.shrink_to_hi(),
                )
            }
...
/// A constant that enters the type system, used for arguments to const generics (e.g. array
/// lengths).
///
...
impl<'hir> ConstArg<'hir, AmbigArg> {
    ...
    ///
    /// In practice this may mean overriding the [`Visitor::visit_infer`][visit_infer] method on hir
    /// visitors, or specifically matching on [`GenericArg::Infer`] when handling generic
    /// arguments.
    ///
...
impl<'hir> ConstArg<'hir> {
    ...
    pub fn try_as_ambig_ct(&self) -> Option<&ConstArg<'hir, AmbigArg>> {
        ...
        // SAFETY: `ConstArg` is `repr(C)` and `ConstArgKind` is marked `repr(u8)` so that the
        // layout is the same across different ZST type arguments. We also asserted that the
        // `self` is not a `ConstArgKind::Infer` so there is no risk of transmuting a `()`
        // to `AmbigArg`.
...
impl<'hir> GenericArgs<'hir> {
    ...
    /// Obtain the list of input types and the output type if the generic arguments are
    /// parenthesized.
    ///
    ...
    pub fn paren_sugar_inputs_output(&self) -> Option<(&[Ty<'hir>], &Ty<'hir>)> {
        ...
        let inputs = self
            ...
            .find_map(|arg| {
                ...
                let TyKind::Tup(tys) = &ty.kind else {
                    return None;
                };
                Some(tys)
            })
    ...
    pub fn has_err(&self) -> Option<ErrorGuaranteed> {
        self.args
            ...
            .find_map(|arg| {
                ...
                let TyKind::Err(guar) = ty.kind else {
                    return None;
                };
                Some(guar)
            })
            .or_else(|| {
                self.constraints.iter().find_map(|constraint| {
                    let TyKind::Err(guar) = constraint.ty()?.kind else {
                        return None;
                    };
                    Some(guar)
                })
    ...
    pub fn span(&self) -> Option<Span> {
        ...
        Some(
            span_ext
                .with_lo(span_ext.lo() + BytePos(1))
                .with_hi(span_ext.hi() - BytePos(1)),
        )
    }
...
impl<'hir> Generics<'hir> {
    ...
    pub fn get_named(&self, name: Symbol) -> Option<&GenericParam<'hir>> {
        self.params
            .iter()
            .find(|&param| name == param.name.ident().name)
    }
    ...
    pub fn span_for_param_suggestion(&self) -> Option<Span> {
        self.params
            .iter()
            .any(|p| self.span.contains(p.span))
            .then(|| {
                // `fn foo<A>(t: impl Trait)`
                //          ^ suggest `, T: Trait` here
                self.span
                    .with_lo(self.span.hi() - BytePos(1))
                    .shrink_to_lo()
            })
    }
    ...
    pub fn bounds_for_param(
        ...
        self.predicates
            .iter()
            .filter_map(move |pred| match pred.kind {
    ...
    pub fn outlives_for_param(
        ...
        self.predicates
            .iter()
            .filter_map(move |pred| match pred.kind {
                WherePredicateKind::RegionPredicate(rp) if rp.is_param_bound(param_def_id) => {
                    Some(rp)
                }
                _ => None,
            })
    ...
    pub fn bounds_span_for_suggestions(
        ...
        self.bounds_for_param(param_def_id)
            .flat_map(|bp| bp.bounds.iter().rev())
            .find_map(|bound| {
                ...
                span_for_parentheses.map_or_else(
                    || {
                        ...
                        // code, as we use this method to get a span
                        // appropriate for suggestions.
                        let bs = bound.span();
                        bs.can_be_used_for_suggestions()
                            .then(|| (bs.shrink_to_hi(), None))
                    },
...
impl<'hir> WhereBoundPredicate<'hir> {
    ...
    pub fn is_param_bound(&self, param_def_id: DefId) -> bool {
        self.bounded_ty
            .as_generic_param()
            .is_some_and(|(def_id, _)| def_id == param_def_id)
    }
}
...
impl<'hir> Block<'hir> {
    pub fn innermost_block(&self) -> &Block<'hir> {
        let mut block = self;
        while let Some(Expr {
            kind: ExprKind::Block(inner_block, _),
            ..
        }) = block.expr
        {
            block = inner_block;
...
impl<'hir> Pat<'hir> {
    fn walk_short_(&self, it: &mut impl FnMut(&Pat<'hir>) -> bool) -> bool {
        ...
        match self.kind {
            ...
            Slice(before, slice, after) => before
                .iter()
                .chain(slice)
                .chain(after.iter())
...
/// Represents a `let <pat>[: <ty>] = <expr>` expression (not a [`LetStmt`]), occurring in an
/// `if-let` or `let-else`, evaluating to a boolean. Typically the pattern is refutable.
///
    ...
    pub fn is_place_expr(&self, mut allow_projections_from: impl FnMut(&Self) -> bool) -> bool {
        match self.kind {
            ExprKind::Path(QPath::Resolved(_, ref path)) => {
                matches!(
                    path.res,
                    Res::Local(..) | Res::Def(DefKind::Static { .. }, _) | Res::Err
                )
            }
                ...
                Expr {
                    kind:
                        ExprKind::Path(QPath::Resolved(
                            None,
                            Path {
                                res: Res::Def(DefKind::Ctor(_, CtorKind::Fn), _),
                                ..
                            },
                        )),
                    ..
...
pub enum AssocItemConstraintKind<'hir> {
    /// An equality constraint for an associated item (e.g., `AssocTy = Ty` in `Trait<AssocTy =
    /// Ty>`).
    ///
    /// Also known as an *associated item binding* (we *bind* an associated item to a term).
    ///
    /// Furthermore, associated type equality constraints can also be referred to as *associated
    /// type bindings*. Similarly with associated const equality constraints and *associated
    /// const bindings*.
...
pub enum TyKind<'hir, Unambig = ()> {
    ...
    TraitObject(
        ...
        TaggedRef<'hir, Lifetime, TraitObjectSyntax>,
    ),
    /// Unused for now.
...
impl InlineAsm<'_> {
    pub fn contains_label(&self) -> bool {
        self.operands
            .iter()
            .any(|x| matches!(x.0, InlineAsmOperand::Label { .. }))
    }
}
...
impl<'hir> Node<'hir> {
    ...
    pub fn associated_body(&self) -> Option<(LocalDefId, BodyId)> {
        match self {
            ...
            Node::Expr(Expr {
                kind: ExprKind::Closure(Closure { def_id, body, .. }),
                ..
            }) => Some((*def_id, *body)),
