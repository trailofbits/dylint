...
pub fn config_toml(name: &str) -> ConfigResult<Option<toml::Value>> {
    let Some(config_table) = CONFIG_TABLE.get() else {
        return Err(ConfigErrorInner::Other(
...
pub fn try_init_config(sess: &rustc_session::Session) -> ConfigResult<()> {
    ...
    // smoelius: Rewrite this function (`try_init_config`) and eliminate the next `expect` once
    // `get_or_try_init` stabilizes: https://github.com/rust-lang/rust/issues/109737
    CONFIG_TABLE
        .set(table.unwrap_or_default())
