...
impl<'tcx> LateLintPass<'tcx> for MissingDocCommentLlm {
    fn check_crate(&mut self, _cx: &LateContext<'tcx>) {
        // API key checking is delegated to litellm-rs, which will look for the appropriate
        // provider-specific environment variable (e.g., ANTHROPIC_API_KEY, OPENAI_API_KEY, etc.)
