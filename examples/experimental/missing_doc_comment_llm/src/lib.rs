...
dylint_linting::impl_late_lint! {
    ...
    /// ### Why is this bad?
    ///
    /// Understanding what a function does is easier given a description of the function rather
    /// than just its code.
    ///
    /// ### Known problems
    ///
    ...
    /// ### LLM Integration
    ///
    /// The lint will suggest a doc comment generated by an LLM via the [litellm-rs] library. The
    /// appropriate API key environment variable must be set based on the model being used (e.g.,
    /// `ANTHROPIC_API_KEY` for Anthropic models, `OPENAI_API_KEY` for OpenAI models). The prompt
    /// sent to the LLM has the following form:
    ///
    /// ````ignore
    /// ```rust
    ...
    /// ### Configuration
    ///
    /// Certain [LLM parameters] can be configured by setting them in the `missing_doc_comment_llm`
    ...
    /// - `temperature` (default 0.2). Note that this default is less than the typical default
    ///   (1.0). Per the [`temperature` documentation], "Higher values like 0.8 will make the
