...
#[test]
fn ui_threshold() {
    dylint_testing::ui::Test::src_base(env!("CARGO_PKG_NAME"), &std::path::Path("ui_threshold"))
...
fn ui_main_rs_equal() {
    ...
    let ui_threshold_main_rs =
        std::fs::read_to_string(std::path::Path("ui_threshold/main.rs")).unwrap();
