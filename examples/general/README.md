...
The general-purpose lints use `dylint_linting`'s [`constituent` feature]. This allows the
lints to be built in either of the following two configurations:

- as individual libraries
- as a single, combined `general` library

However, some additional organization of both the `general` directory and its subdirectories
is required.
...
- Each lint's `register_lints` function is called from the `register_lints` function in
  src/lib.rs.
...
- The following files do not appear, even though they would be created by
  ...
  - .gitignore
  - rust-toolchain
- The lint's Cargo.toml has the following `[lib]` section (note: `crate-type` is not just
  `["cdylib"]`):
  ```toml
  [lib]
