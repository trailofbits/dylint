...
Checks for calls to await while holding a `tracing` span's `Entered` or `EnteredSpan`
guards.
...
async block yields at an .await point, the current scope is exited, but values in that
scope are not dropped (because the async block will eventually resume execution from that
